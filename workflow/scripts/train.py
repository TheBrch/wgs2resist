...
def define_model(name):
    match name:
        ...
        #         'kernel_type',
        #         ['rbf', 'matern', 'rational_quadratic', 'rbf_white']
        #     )

        #     match kernel_type:
        #         case 'rbf_white':
        #             kernel = RBF(length_scale=trial.suggest_float('length_scale', 0.1, 10, log=True)) + \
        #                 WhiteKernel(noise_level=trial.suggest_float('noise_level', 1e-10, 1e-3, log=True))
        #         case 'rbf':
        #             kernel = RBF(length_scale=trial.suggest_float('length_scale', 0.1, 10, log=True))
        #         case 'matern':
        #             kernel = Matern(
        #                 length_scale=trial.suggest_float('length_scale', 0.1, 10, log=True),
        #                 nu=trial.suggest_categorical('nu', [0.5, 1.5, 2.5])
        #             )
        #         case 'rational_quadratic':
        #             kernel = RationalQuadratic(
        #                 length_scale=trial.suggest_float('length_scale', 0.1, 10, log=True),
        #                 alpha=trial.suggest_float('rq_alpha', 0.1, 10, log=True)
        #             )
        #         case _:
        #             raise ValueError(f"Unknown GP Kernel chosen: {kernel_type}")
